...
# Allow a browser page to call the API (we can lock this down later)
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],            # change to your static site URL later
    allow_credentials=True,
    allow_methods=["*"],
    allow_headers=["*"],
)

# Optional: if you want /deploy to ping a Pipedream workflow, put its URL
# in your Render "Environment" as PD_WEBHOOK_URL. If empty, we just no-op.
...
async def send_to_pipedream(payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        return {"status_code": 599, "error": repr(e)}
